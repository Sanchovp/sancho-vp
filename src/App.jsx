...
function greetingKickoff(personaKey, firstName) {
  ...
  if (personaKey === "sandra") {
    return `${base} Como parte da apresentação, informe a situação real do checklist de documentos (contexto acima): se ele ainda não foi configurado (vazio), diga isso claramente e ofereça ajuda para montar um checklist inicial — nunca diga que "está tudo certo" ou "não falta nada" nesse caso, pois isso seria enganoso. Se houver itens pendentes, liste-os. Se houver itens no checklist e todos já estiverem recebidos, aí sim parabenize. Mencione também que o usuário pode enviar qualquer outra informação relevante para você organizar.`;
  }
  if (personaKey === "savio") {
